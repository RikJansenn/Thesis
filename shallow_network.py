...
def create_model():
    if IP:
        reservoir = IPReservoir(500, mu=0.0, sigma=0.1, sr=0.95, activation="tanh", epochs=10)
    else:
        reservoir = Reservoir(500, lr=0.5, sr=0.9)
    readout = Ridge(ridge=1e-7)
    return reservoir, readout

def pretrain_model(reservoir):
    x = narma(1000)
    reservoir = reservoir.fit(x, warmup=100)
    return reservoir
...
if __name__ == "__main__":
    ...
    if IP:
        reservoir = pretrain_model(reservoir)
