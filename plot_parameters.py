...
def plot_parameters_against_measure(measure):
    ...
    sns.stripplot(data=data, x="sr", y=measure,  hue="lr", palette="viridis", ax=axes[1], alpha=0.5, jitter=True)
    axes[1].set_title(f"{measure} against spectral radius")

    sns.boxplot(data=data, x="lr", y=measure, ax=axes[2])
    sns.stripplot(data=data, x="lr", y=measure, hue="sr", palette="viridis", ax=axes[2], alpha=0.5, jitter=True)
